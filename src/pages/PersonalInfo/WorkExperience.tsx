import { FlatList, Image, TouchableOpacity, ScrollView, StyleSheet, TouchableWithoutFeedback, View, ActivityIndicator } from 'react-native'
            ...
            <>
              {
                cvs?.length > 0 ? <>
                  <FlatList scrollEnabled={false} data={cvs} style={{ flex: 1, width: responsiveScreenWidth(90) }} renderItem={({ item, index }) => {
                    return (
                      <>
                        <CvCard refresh={() => dispatch(GetExperience()).unwrap().then(res => {
                          if (res.success)
                            setCvs(res.data)
