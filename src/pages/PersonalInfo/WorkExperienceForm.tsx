...
const WorkExperienceForm = () => {
  ...
  const onSubmit = () => {
    if (!formData.experienceTitle.trim()) {
      showAlert({
        title: "Missing Experience Title",
        message: "Please enter your job title or role.",
      });
      return;
    }
    ...
    if (!formData.country) {
      showAlert({
        ...
        message: "Please select a country where you worked.",
      });
      return;
    }
    ...
    if (!formData.description.trim()) {
      showAlert({
        ...
        message: "Please add a brief description of your work experience.",
      });
      return;
    }

    if (!formData.currentlyWorking && formData.endDate < formData.startDate) {
      showAlert({
    ...
    if (data?.id) {
      dispatch(EditWorkExperience({
        ...
        date_start: formData.startDate.toISOString().slice(0, 19).replace('T', ' '),
        date_end: formData.currentlyWorking
          ? null
          : formData.endDate.toISOString().slice(0, 19).replace('T', ' '),
