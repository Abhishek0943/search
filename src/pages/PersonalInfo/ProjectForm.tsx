...
const updateProjectWithFetch = async (fd: FormData, navigation: any, fun, id) => {
  try {
    const token = await AsyncStorage.getItem('token');
...
const ProjectForm = () => {
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({
      ...
      projectUrl: data.url ?? prev.projectUrl,
      startDate: data.date_start ? new Date(data.date_start) : prev.startDate,
