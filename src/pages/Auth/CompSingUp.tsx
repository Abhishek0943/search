...
const CompSingUp = () => {
    ...
    return (
        <>
            <ComponentSingUp type={"comp"} mainColor={colors.compPrimary} secondaryColor={colors.compPrimary2} />
        </>
    )
}
...
const getPasswordStrength = (password: string, mainColor: string): {
    ...
    if (password.length === 0) {
        return { strength: 'none', label: '', color: '#DEDEDE', bars: 0, hasMinLength, hasNumberOrSymbol };
    }
    ...
    if (score <= 1) {
        return { strength: 'weak', label: 'Weak', color: '#E74C3C', bars: 1, hasMinLength, hasNumberOrSymbol };
    } else if (score === 2) {
        return { strength: 'good', label: 'Good', color: '#F39C12', bars: 2, hasMinLength, hasNumberOrSymbol };
    } else {
        return { strength: 'strong', label: 'Strong', color: mainColor, bars: 3, hasMinLength, hasNumberOrSymbol };
    }
};

const PasswordStrengthIndicator = ({ password, mainColor, secondaryColor }: { password: string; mainColor: string, secondaryColor: string }) => {
    const { colors } = useContext(ThemeContext);
    const info = useMemo(() => getPasswordStrength(password, secondaryColor), [password, secondaryColor]);
    ...
    return (
        <View style={{ marginTop: responsiveHeight(0.8), }}>
            ...
            <View style={{ gap: responsiveHeight(0.4) }}>
                <RequirementRow color={info.hasMinLength ? secondaryColor : colors.textSecondary} text="At least 8 characters" />
                <RequirementRow color={info.hasNumberOrSymbol ? secondaryColor : colors.textSecondary} text="One number or symbol" />
            </View>
        </View>
    );
};
    ...
    return (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: responsiveWidth(1.5) }}>
            <View style={{ aspectRatio: 10 / 7.2, width: responsiveWidth(3) }}>
                <Image tintColor={color} style={{ height: "100%", width: "100%", }} source={imagePath.Check2} />
            </View>
            <Text
                style={{
                    fontSize: responsiveFontSize(1.6),
...
export const ComponentSingUp = ({ mainColor, secondaryColor, type = "jobSeeker", emailText = "A business address speeds up verification later." }: {
    mainColor: string,
                ...
                return (
                    <>
                        ...
                        <Pressable onPress={() => {
                            ...
                            <Image style={{ height: "100%", width: "100%", resizeMode: "contain" }} source={type === "comp" ? require("./CompVerificationCode.png") : require("./SendVerificationCode.png")} />
                        </Pressable></>
                )
            case (2):
                return (
                    <>
                        ...
                        <View style={{
                            ...
                            <Text numberOfLines={1} style={{ color: secondaryColor, fontSize: responsiveFontSize(2), fontWeight: '800', marginRight: responsiveWidth(2) }}>{user.email || "abhishek"}</Text>
                            <Text onPress={() => { setStep(1) }} style={{ color: mainColor, fontSize: responsiveFontSize(1.8), fontWeight: '800', marginRight: responsiveWidth(2) }}>Change</Text>
                        </View>
                        ...
                        <Pressable style={{ width: responsiveWidth(90), marginTop: responsiveHeight(2.5), aspectRatio: 350 / 56, }}>
                            <Image style={{ height: "100%", width: "100%", resizeMode: "contain" }} source={require("./CompVerify.png")} />
                        </Pressable>
                    </>
                )
        }
    ...
    useEffect(() => {
        const a =
            async () => {
                ...
                const response = await fetch(`http://ip-api.com/json/${ip}?fields=2158591`);
    ...
    return (
        <>
            <View style={{ backgroundColor: colors.background, paddingBottom: responsiveHeight(8), height: responsiveHeight(100), width: responsiveWidth(100), flex: 1, paddingVertical: responsiveHeight(5), paddingHorizontal: responsiveWidth(5), }}>
                {renderElem()}
            </View>
