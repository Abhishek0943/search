...
const Company = () => {
  ...
  useEffect(() => {
    if (isFavorite) {
      dispatch(GetFavoriteCompanies()).unwrap().then((res) => {
        if (res.success) {
          setJob(res.data)
        }
        setLoading(false)
      ...
      <ScrollView style={{ flex: 1, }} contentContainerStyle={{ justifyContent: "flex-start" }}>
        <Header title={isFavorite ? "Favorite Companies" : "Companies"} />
        {
          loading ? <>
            ...
            scrollEnabled={false} contentContainerStyle={{ marginHorizontal: responsiveScreenWidth(5), gap: responsiveScreenHeight(1), marginVertical: responsiveScreenHeight(1) }} data={job.companies} renderItem={({ item, index }) => {
              return (
                <>
