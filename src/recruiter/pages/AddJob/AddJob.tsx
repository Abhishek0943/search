...
//i want to create benift and discription seprate state \ 
    ...
    useEffect(() => {
        ...
        if (jobData.country_id) {
            dispatch(GetState({ id: jobData.country_id }))
                .unwrap()
                .then(res => res.success && setStates(res.data));
        }

        if (jobData.state_id) {
            dispatch(GetCity({ id: jobData.state_id }))
                .unwrap()
                .then(res => res.success && setCities(res.data));
        }
    }, [jobData, dispatch]);
    const onSubmit = () => {
        if (isLoading) return;
        const requiredChecks = [
            ...
            { value: dis.description, label: "Job Description" },
            { value: dis.benefits, label: "Benefits" },
                            ...
                            user.package_end_date && new Date(user.package_end_date.replace(' ', 'T')) >= new Date() ? <>
                                ...
                                <View style={[{ width: '100%', minHeight: responsiveScreenHeight(20) }, { ...inputStyle, paddingHorizontal: 0, paddingVertical: 0 }]}>
                                    <RichEditor
                                        ...
                                        onChange={html => setDis({ ...dis, "description": html })}
                                        initialContentHTML={dis.description}
                                        editorStyle={{
                                ...
                                <View style={[{ width: '100%', minHeight: responsiveScreenHeight(20) }, { ...inputStyle, paddingHorizontal: 0, paddingVertical: 0 }]}>
                                    <RichEditor
                                        ...
                                        onChange={html => setDis({ ...dis, "benefits": html })}
                                        // onInitialized={() => setBenReady(true)}
                                        initialContentHTML={dis.benefits}
                                        editorStyle={{
                                            backgroundColor: 'transparent',
                                            color: colors.textPrimary,
                                            placeholderColor: colors.gray,
                                ...
                                <DatePicker
                                    ...
                                    maximumDate={new Date(user?.package_end_date?.replace(' ', 'T'))}
                                    mode="date"
                                    onConfirm={(date) => {
